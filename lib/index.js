...
const { join, normalize, sep, dirname, basename } = require("path");
 ...
 */
function transliteratePinyinSiderbar(sidebar) {
  if (sidebar.length) {
    // array
    return sidebar.map(sidebar => {
      if (typeof(sidebar) == 'string') {
        return slugify(sidebar, { ignore: ["/", "."] });
      } else if (sidebar.children) {
        sidebar.children = transliteratePinyinSiderbar(sidebar.children);
      }
      return sidebar;
    });
  } else {
    ...
    for (let path in sidebar) {
      ...
      sidebar[newPath] = transliteratePinyinSiderbar(sidebar[path]);
      delete sidebar[path];
    }
    return sidebar;
  }
...
function getChildren(parent_path, dir, filter, recursive = true) {
  ...
  files = glob.sync(parent_path + (dir ? `/${dir}` : "") + pattern).map(path => {
    ...
    if (filter && !filter(md.meta)) return undefined
...
function nameOfFirstStringChild(child) {
  if(typeof child == 'string') return child;
  else if (typeof child == 'object' && child.children) return nameOfFirstStringChild(child.children[0])
  else return "";
}
...
function side(
  baseDir,
  { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, addReadMeToFirstGroup, mixDirectoriesAndFilesAlphabetically, filter },
  relativeDir = "",
  currentLevel = 1
) {
  const fileLinks = getChildren(baseDir, relativeDir, filter, currentLevel > maxLevel);
        ...
        if (children.length > 0 || !skipEmptySidebar) {
          ...
          const sortedFolderPosition = fileLinks.findIndex(
            link => {
              ...
              if(link.children) {
                let childrenTitle = nameOfFirstStringChild(link.children[0])
              ...
              return relativeDir === "" ? subDir < linkLabel : relativeDir + sep + subDir < linkLabel
 ...
 * @param   {String}    rootDirOrOptions  - Directory to get configuration for or options.
 * @param   {Object}    options           - Options
 * @returns {Object}                      - { nav: ..., sidebar: ... } configuration.
 */
function getConfig(rootDirOrOptions, options) {
  let rootDir = typeof rootDirOrOptions === "string" ? normalize(rootDirOrOptions) : join(dirname(parentModule()), '..');
