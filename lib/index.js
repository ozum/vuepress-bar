          ...
          const sortedFolderPosition = fileLinks.findIndex(
            link => {
              ...
              if(link.children) {
                ...
                return subDir < childrenTitle.split(sep)[0];
              }
              return subDir < link;
            }
          );
...
function getConfig(rootDirOrOptions, options) {
  ...
  if(options.groupPrefix && nav.length) {
    result.nav = supportNavGroup(result.nav, options);
  }
...
/**
 * This function could support multi level group，but vuepress only support one level group
 *
 * @param {*} navItems
 * @param {*} options
 * @return {*}
 */
function supportNavGroup(navItems, options) {
  if(navItems && navItems.length) {
    navItems = getGroupName(navItems, options);
    const groups = [];
    const groupsMap = {};
    navItems.forEach(item => {
      if(item.items && item.items.length) {
        item.items = supportNavGroup(item.items, options)
      }
      if(item.groupName) {
        if(groupsMap[item.groupName]) {
          groupsMap[item.groupName].items.push(item);
        } else {
          groupsMap[item.groupName] = {
            text: item.groupName,
            items: [item]
          }
          groups.push(groupsMap[item.groupName])
        }
      } else {
        groups.push(item)
      }
...
/**
 * we can support group nav inside directory name
 * eg：nav.01.group.groupName.navName
 *
 * @param {*} navItems
 * @param {*} options
 * @return {*}
 */
function getGroupName(navItems, options) {
  return navItems.map(item => {
    const isGroup = item.text.toLowerCase().startsWith(options.groupPrefix + '.');
    if(isGroup) {
      const splitNames = item.text.split('.');
      item.groupName = splitNames[1];
      item.text = titleize(splitNames[2]);
    } else {
      item.groupName = '';
    }
    return item;
