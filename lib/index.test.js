...
describe("getConfig", () => {
  ...
  it('sort correctly when subsection is alphabetically before file.', async () => {
    expect(configOf('sort2')).toEqual(expected('sort2'))
  })
  ...
  it("should filter based on meta data.", () => {
    expect(configOf("filter", { filter : (meta) => meta.draft !== true })).toEqual(expected("filter"));
  });
});
